...
```{r downloadMet}
##This downloads 3.3Gb of data
downloadList <- c(
  sprintf('ftp://ftp.cdc.noaa.gov/Datasets/cpc_global_precip/precip.%d.nc', 1979:2018),
  #sprintf('ftp://ftp.cdc.noaa.gov/Datasets/cpc_global_temp/tmax.%d.nc', 1979:2018),
...
downloadList <- downloadList[!file.exists( sprintf('~/Documents/Datasets/CPC/%s',
                                  basename(downloadList)))]

for(downloadFile in downloadList){
  download.file(downloadFile, 
              sprintf('~/Documents/Datasets/CPC/%s', basename(downloadFile)))
}

#downloadFile <- "https://e4ftl01.cr.usgs.gov/MOLT/MOD17A3.055/2000.01.01/MOD17A3.A2000001.h00v09.055.2011276104218.hdf"
#download.file(downloadFile, 
#              sprintf('~/Documents/Datasets/CPC/%s', basename(downloadFile)))
#MODIS products need to be downloaded invidually
#http://neo.sci.gsfc.nasa.gov/servlet/RenderData?si=1197278&cs=gs&format=TIFF&width=3600&height=1800
#MOD17A3
#https://neo.sci.gsfc.nasa.gov/view.php?datasetId=MOD17A2_M_PSN&date=2000-12-01
```

```{r loadMet}
datasetDir <- '~/Documents/Datasets/CPC'
loadFiles <- list(Tmin=sprintf('%s/tmin.day.ltm.nc', datasetDir),
                  Tmax=sprintf('%s/tmax.day.ltm.nc', datasetDir))
...
latLon <- soil.df %>% dplyr::select(lon, lat) %>% unique %>%
  group_by(lon, lat) %>%
...
precipByYear <- data.frame(year = 1979:2017, stringsAsFactors=FALSE) %>%
  group_by(year) %>%
  do((function(xx){
    precip.nc <- nc_open(sprintf('%s/precip.%d.nc', datasetDir, xx$year))
    tempPrecip <- ncvar_get(precip.nc, varid='precip')
    nc_close(precip.nc)
    precipPatterns <- latLon %>%
      group_by(nearLon, nearLat) %>%
      dplyr::select(nearLon, nearLat) %>% unique() %>%
      do((function(x){
        precip <- tempPrecip[x$nearLon, x$nearLat,]
        if(all(is.finite(precip) & precip == 0)){
          return(data.frame(meanLength_day = 0,
                            rangeLength_day=0, 
                            mean_precip=0, 
                            rangePrecip=0, 
                            total_precip=0,
        ...
        if(any(is.na(precip))){
          return(data.frame(meanLength_day = as.numeric(NA),
                            rangeLength_day= as.numeric(NA), 
                            mean_precip= as.numeric(NA), 
                            rangePrecip= as.numeric(NA), 
                            total_precip= as.numeric(NA),
                            eventCount= as.numeric(NA)))
        }
        
        #return(data.frame(precip[1]))
        ans <- data.frame(precip,
                          eventIndex=cumsum(xor(c(0, precip[-length(precip)]) != 0 , 
                                                precip !=  0))) %>%
          group_by(eventIndex) %>%
          summarize(length_days=length(precip),
                    tot_precip=sum(precip)) %>%
          filter(tot_precip != 0) %>%
          summarize(meanLength_day = mean(length_days),
                    rangeLength_day=diff(range(length_days)), 
                    mean_precip=mean(tot_precip), 
                    rangePrecip=diff(range(tot_precip)), 
                    total_precip=sum(tot_precip),
                    eventCount=length(tot_precip))
        return(ans)
      })(.))
    return(precipPatterns)
  })(.))
...
```{r}
ggplot(precipByYear %>% gather(key='key', value='value', -year, -nearLat, -nearLon)) + 
  geom_line(aes(x=year, y=value, group=sprintf('%d-%d', nearLat, nearLon))) +
...
```{r downloadNPP}
##Download using wget command below
##wget -w 0.5 -l 0 -r -nH -np -nc -I /Y2000/*, /Y2000 -R *.html,*.htm http://files.ntsg.umt.edu/data/NTSG_Products/MOD17/MOD17A3/Y2000

#download.file('ftp://ftp.ntsg.umt.edu/pub/MODIS/NTSG_Products/MOD17/MOD17A3/Y2000/', '~/Documents/Datasets/MODIS17A3/Y2000', method='wget', extra=')
library(raster)
#library(sp)
#library(rgdal)
library(gdalUtils)
#testFile <- '/Users/todd528/Documents/Datasets/MODIS17A3/data/NTSG_Products/MOD17/MOD17A3/Y2000/MOD17A3.A2000366.h00v08.305.2009342143301.hdf'
#sds <- get_subdatasets(testFile)
#library(MODIS)
#MODIS:::checkTools()
#sds <- getSds(testFile)
#r <- raster(testFile)
...
#http://files.ntsg.umt.edu/data/NTSG_Products/MOD17/GeoTIFF/MOD17A3/GeoTIFF_30arcsec/MOD17A3_Science_NPP_Qc_mean_00_15.tif
NPPFile <- "../temp/MOD17A3_Science_NPP_mean_00_15.tif"
QCFile <- "../temp/MOD17A3_Science_NPP_Qc_mean_00_15.tif"
#file.exists(NPPFile)
...
npp.df <- soil.df %>% dplyr::select(lat, lon, mapsID) %>% unique %>%
  mutate(NPP = extract(npp.raster, SpatialPoints(cbind(lon, lat), proj4string=crs(npp.raster))),
         npp.qc = extract(qc.raster, SpatialPoints(cbind(lon, lat), proj4string=crs(npp.raster)))) #%>% rename(x=lon, y=lat)
...
ggplot(npp.df %>% filter(mapsID == 'USA') )+
  ...
  geom_point(aes(x=lon, y=lat, color=log10(NPP)), size=0.1) + 
