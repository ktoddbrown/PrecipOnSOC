...
workingTemp_dir <- '~/Desktop/Working/PrecipOnSOC/temp'
```
...
for(downloadFile in downloadList){
  download.file(downloadFile, file.path(met_dir, basename(downloadList)))
}
...
```{r}
soil.df <- soil.df %>%
  dplyr::left_join(met.df, by=c('lon', 'lat')) 
...
#0=u-k*Q10*S
#u=k*Q10*S
#log(u) = log(k) + log(Q10) + log(S)
##log(Q10) = (T-15)/10*log(2)
#log(u) ~ log(S) + T
#u/(S*Q10)=k
#u/S=k*Q10

ggplot(soil.df %>% filter(npp < 6e4)) +
  geom_point(aes(y=SOC, x=(npp/1e4)), alpha=0.1) +
  scale_y_log10()

ggplot(soil.df %>% filter(npp < 6e4)) +
  geom_point(aes(y=SOC/(npp*1e-4), x=MAT), alpha=0.1) +
  scale_y_log10()

ggplot(soil.df %>% filter(npp < 6e4)) +
  geom_point(aes(y=SOC/(npp*1e-4), x=clay_tot_psa_100), alpha=0.1) +
  scale_y_log10() +
  facet_wrap(~hzn, scales='free_x')

ggplot(soil.df %>% filter(npp < 6e4)) +
  geom_violin(aes(y=SOC/(npp*1e-4), x=factor(round(clay_tot_psa_100/10)))) +
  scale_y_log10()

ggplot(soil.df %>% filter(npp < 6e4))+
  geom_point(aes(y=(SOC*2^((MAT-15)/10))/(npp*1e-4), x=clay_tot_psa_100), alpha=0.1) + 
  scale_y_log10()

```

```{r soilTaxon}
print(head(soil.df$soil_taxon[5:10]))
print(paste('Missing soil taxon: ', sum(is.na(soil.df$soil_taxon)), 'of', nrow(soil.df)))
soilTaxon_words <- data.frame(words=unlist(strsplit(as.character(unique(soil.df$soil_taxon )), split=',?\\s+'))) %>%
  group_by(words) %>%
