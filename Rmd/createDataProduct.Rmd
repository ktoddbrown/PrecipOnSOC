...
data.df <- soil.df %>%
  ...
  mutate(k_npp=npp/SOC,
         k_nppQ10 = npp/(SOC*2^((MAT-15)/10)))
```
```{r pairWiseComp}
GGally::ggpairs(data.df, columns=c('MAT', 'MAP', 'MAGini', 'npp', 'SOC'))
GGally::ggpairs(data.df %>% filter(k_npp < 3e6), columns=c( 'MAT', 'MAP', 'MAGini','k_npp'))
GGally::ggpairs(data.df %>% filter(k_nppQ10 < 3e6), columns=c('MAP', 'MAGini', 'k_nppQ10'))
...
GGally::ggpairs(data.df %>% filter(k_nppQ10 < 3e6,
                                   grepl('argi', soil_taxon)), columns=c('MAP', 'MAGini', 'k_nppQ10'))
GGally::ggpairs(data.df %>% filter(k_nppQ10 < 3e6,
                                   grepl('(montmor)|(smectit)', soil_taxon)), columns=c('MAP', 'MAGini', 'k_nppQ10'))
